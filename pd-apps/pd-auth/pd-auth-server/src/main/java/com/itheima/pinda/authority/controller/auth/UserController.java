...
import com.itheima.pinda.authority.biz.converter.RoleDtoConverter;
...
public class UserController extends BaseController {
    ...
    public R<IPage<User>> page(UserPageDTO userPage) {
        ...
        User user = userDtoConverter.userPage2Dto(userPage);
...
//        userService.page(page, wrapper);
    ...
    public R<User> save(@RequestBody @Validated UserSaveDTO data) {
        User user = userDtoConverter.userSave2Dto(data);
    ...
    public R<User> update(@RequestBody @Validated(SuperEntity.Update.class) UserUpdateDTO data) {
        User user = userDtoConverter.userUpdate2Dto(data);
    ...
    public R<User> avatar(@RequestBody @Validated(SuperEntity.Update.class) UserUpdateAvatarDTO data) {
        User user = userDtoConverter.userUpdateAvatar2Dto(data);
        userService.updateUser(user);
        return success(user);
    }
    ...
    public R<SysUser> getById(@PathVariable Long id, @RequestBody UserQuery query) {
        ...
        SysUser sysUser = userDtoConverter.sysUser2Dto(user);

        if (query.getFull() || query.getOrg()) {
            sysUser.setOrg(userDtoConverter.sysOrg2Dto(orgService.getById(user.getOrgId())));
        }
        if (query.getFull() || query.getStation()) {
            sysUser.setStation(stationDtoConverter.sysStation2Dto(stationService.getById(user.getStationId())));
        }

        if (query.getFull() || query.getRoles()) {
            List<Role> list = roleService.findRoleByUserId(id);
            sysUser.setRoles(roleDtoConverter.roleList2Dto(list));
        }
