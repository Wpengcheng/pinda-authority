...
public class AccessFilter extends BaseFilter{
	...
	public Object run() {
		...
		if(resourceNeed2Auth != null){
			long count = resourceNeed2Auth.stream().filter(permission::startsWith).count();
			if(count == 0){
				//未知请求
		...
		if(userResource == null){
			...
			if(result.getData() != null){
				...
				userResource = userResourceList.stream().map( r -> r.getMethod() + r.getUrl()).collect(Collectors.toList());
